...
namespace Cafe_Management.AllUserControls
{
    public partial class UC_RemoveItem : UserControl
    {
        ...
        {
            query = "select name as 'Name',category as 'Category',price as 'Unit Price'from items";
        ...
        {
            query = "select name,category,price from items where name like '%"+txtSearch.Text+"%' ";
            DataSet ds = fn.getData(query);
            guna2DataGridView1.DataSource = ds.Tables[0];
        }
